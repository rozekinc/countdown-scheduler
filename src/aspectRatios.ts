export interface AspectRatioPreset {
  id: string;
  label: string;
  /** The stage is letterboxed/pillarboxed to this width:height ratio
   * regardless of the physical screen's own ratio -- same idea as
   * broadcast-TV letterboxing, just parameterized instead of hardcoded to
   * 16:9. See styles.css's .aspect-inner (--stage-w/--stage-h). */
  w: number;
  h: number;
}

// Keep this list, and every value in it, identical to
// admin-src/aspectRatios.ts.
